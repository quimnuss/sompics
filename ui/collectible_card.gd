...
func refresh_collectible():
	...
	if Persistence.censored_version:
		title_text.text = 'Això era privat :P'
		equip_text.text = ''
		time_text.text = ''
		impacte_text.text = ''
		how_text.text = ''
		why_text.text = ''
		impacte_text.text = 'Les fites eren de la cooperativa :D algun dia en posaré de noves pels lols'
	else:
